...
import static q.rorbin.verticaltablayout.util.MapUtil.isEqualCollection;
...
public class VerticalTabLayout extends ScrollView {
    ...
    public VerticalTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        mIndicatorHeight = (int) typedArray.getDimension(R.styleable.VerticalTabLayout_indicator_height, DisplayUtil.dp2px(context, 15));
    ...
    public void removeAllTabs() {
        ...
        mSelectedTab = null;
        for (int i = 0; i < qTabViewList.size(); i++) {
            qTabViewList.get(i).builder_bad = null;
            qTabViewList.get(i).builder_tab = null;
            qTabViewList.get(i).builder_title = null;
        }
        qTabViewList.clear();

        for (int i = 0; i < builders.size(); i++) {
            QTabView.TabTitle.Builder builder = builders.get(i);
            builder = null;
        }
        builders.clear();
    ...
    public void setTabSelected(final int position, String action) {
        this.action = action;
        setTabSelected(position, true, true, action);
    }
    ...
    private void setTabSelected(final int position, final boolean updataIndicator, final boolean callListener,
        ...
        post(new Runnable() {
            @Override
            public void run() {
                setTabSelectedImpl(position, updataIndicator, callListener, action);
            }
        });
    }
     ...
     * 更新Tablayout背景
     */
    public void ungradeTabback(int position, final String action) {
        for (int i = 0; i < mTabStrip.getChildCount(); i++) {
            QTabView view = (QTabView) mTabStrip.getChildAt(i);
            view.setBackground(R.drawable.addhandscene_open_shape_gray);
            if (i == position) {
                view.setBackground(R.drawable.addhandscene_open_shape_white);
            }
            if (i == position - 1) {
                view.setBackground(R.drawable.addhandscene_open_shape_gray_bottom);
            }

            if (i == position + 1) {
                view.setBackground(R.drawable.addhandscene_open_shape_gray_up);
            }
        ...
        mTabStrip.post(new Runnable() {
            @Override
            public void run() {
                mTabStrip.updataIndicator(action);
            }
        });
    }


    private void setTabSelectedImpl(final int position, boolean updataIndicator, boolean callListener, String action) {
        if (onTabClickListener != null) onTabClickListener.onTabSelected(null, position);
        ungradeTabback(position, action);
        ...
        if (selected = (view != mSelectedTab)) {
            ...
            if (updataIndicator) {
                mTabStrip.moveIndicatorWithAnimator(position, action);
            }
            mSelectedTab = view;
...
//    }
    ...
    public boolean setTabAdapter(TabAdapter adapter) {

        if (adapter != null) {
            List<Map> list = new ArrayList<>();
            if (list_map.size() != 0) {
                list = adapter.getList();
                if (isEqualCollection(list_map, list)) return true;
                list_map = list;
            } else {
                list_map = adapter.getList();
            }
        ...
        if (adapter != null) {
            ...
            for (int i = 0; i < adapter.getCount(); i++) {
                ...
                qTabViewList.add(qTabView);

                addTab(qTabView.setIcon(adapter.getIcon(i))
...
//                    }
    ...
    List<QTabView> qTabViewList = new ArrayList<>();
    List<QTabView.TabTitle.Builder> builders = new ArrayList<>();

    private void populateFromPagerAdapter() {
        if (mPagerAdapter != null) {
            final int adapterCount = mPagerAdapter.getCount();
            if (mPagerAdapter instanceof TabAdapter) {
                if (setTabAdapter((TabAdapter) mPagerAdapter)) return;
            } else {
                removeAllTabs();
                for (int i = 0; i < adapterCount; i++) {
                    ...
                    QTabView.TabTitle.Builder builder = new QTabView.TabTitle.Builder();
                    builders.add(builder);

                    addTab(qTabView.setTitle(
                            builder.setContent(title).build()));
                }
            }
    ...
    private class TabStrip extends LinearLayout {
        ...
        private RectF mIndicator_before_Rect;
        ...
        public TabStrip(Context context) {
            ...
            mIndicator_before_Rect = new RectF();
            setIndicatorGravity();
        }
        ...
        protected void updataIndicator(String action) {
            moveIndicatorWithAnimator(getSelectedTabPosition(), action);
        }
         ...
         */
        protected void moveIndicatorWithAnimator(int index, String ac) {
            ...
            switch (action) {
                ...
                default:
                    ...
                    post(new Runnable() {
                             ...
                             public void run() {
                                 if (direction > 0) {
                                     mIndicatorBottomY = targetBottom;
                                     invalidate();
                                 } else {
                                     mIndicatorTopY = targetTop;
                                     invalidate();
                                 }
                             }
                         }
...
//            mIndicatorRect.bottom = mIndicatorBottomY;
            Drawable drawable = getResources().getDrawable(R.drawable.addhandscene_open_shape_sraum);
            Rect rect = new Rect();
            rect.left = (int) mIndicatorX;
            int height = (int) (mIndicatorBottomY - mIndicatorTopY);
            rect.top = (int) mIndicatorTopY + height / 4;
            rect.right = (int) (mIndicatorX + mIndicatorWidth);
            rect.bottom = (int) mIndicatorBottomY - height / 4;
            drawable.setBounds(rect);
            Log.e("TAG", "onDraw: left:"  + rect.left
            + ",top:" + rect.top
            + ",right:" + rect.right
            + ",bottom:" + rect.bottom);


//            mIndicator_before_Rect.left = mIndicatorX;
//            mIndicator_before_Rect.top = mIndicatorTopY;
//            mIndicator_before_Rect.right = 3;
//            mIndicator_before_Rect.bottom = mIndicatorBottomY;
//
//
//
//            mIndicatorRect.left = mIndicatorX + 3;
//            mIndicatorRect.top = mIndicatorTopY + 10;
//            mIndicatorRect.right = mIndicatorX + 30;
//            mIndicatorRect.bottom = mIndicatorBottomY + 10;


            // mIndicator_before_Rect.set(rect);


            if (mIndicatorCorners != 0) {
                //  canvas.drawRoundRect(mIndicator_before_Rect, 20, 20, mIndicatorPaint);
                //canvas.drawRoundRect(mIndicatorRect, 20, 20, mIndicatorPaint);
            } else {
