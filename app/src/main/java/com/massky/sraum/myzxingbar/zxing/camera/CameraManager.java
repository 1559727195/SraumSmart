...
package com.massky.sraum.myzxingbar.zxing.camera;
...
public final class CameraManager {
  ...
  private static int MAX_FRAME_HEIGHT = 720;
   ...
   */
  public void openDriver(SurfaceHolder holder, int screen_width, int screen_height) throws IOException {
    if (camera == null) {
      camera = Camera.open();
      if (camera == null) {
        throw new IOException();//1176,2328
      }
...
//      }
      ...
      //changePreviewSize(camera,screen_width,screen_height);
    }
  }
  ...
  protected void changePreviewSize(Camera camera, int viewWidth, int viewHeight) {
    Log.e("cha", "SurfaceView：" + viewWidth + "*" + viewHeight);
    if (camera == null) {
      return;
    }
    ...
    if (closelySize != null) {
      Log.e("cha", "预览尺寸修改为：" + closelySize.width + "*" + closelySize.height);
