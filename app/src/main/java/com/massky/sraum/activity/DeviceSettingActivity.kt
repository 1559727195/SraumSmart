 ...
 */
class DeviceSettingActivity : BaseActivity(), XListView.IXListViewListener {
    ...
    private fun sraum_getAllArea() {
        ...
        mapdevice["token"] = TokenUtil.getToken(this@DeviceSettingActivity)
        ...
        MyOkHttp.postMapString(ApiHelper.sraum_getAllArea, mapdevice, object : Mycallback(AddTogglenInterfacer { //刷新togglen数据
            sraum_getAllArea()
        }, this@DeviceSettingActivity, dialogUtil) {
...
//                            }
                    if (add_room == null) return
                    add_room!!.visibility = View.GONE
                    homesettingadapter = MyAreaListAdapter(this@DeviceSettingActivity, areaList, "deviceManager")
    ...
    private fun sraum_getRoomsInfo(areaNumber: String, authType: String) {
        ...
        MyOkHttp.postMapString(ApiHelper.sraum_getRoomsInfo, mapdevice, object : Mycallback(AddTogglenInterfacer { //刷新togglen数据
            ...
            override fun wrongBoxnumber() {
                ToastUtil.showToast(this@DeviceSettingActivity, """
     areaNumber
     不存在
     """.trimIndent())
            }
    ...
    private fun display_room_list() {
        manageroomadapter = ManagerRoomAdapter(this@DeviceSettingActivity, roomList)
    ...
    override fun onClick(v: View) {
        when (v.id) {
            R.id.back -> finish()
            R.id.btn_cancel_wangguan, R.id.add_room -> startActivity(Intent(this@DeviceSettingActivity, AddAreaActivity::class.java))
        }
    }
