...
//        rid = mapdevice.get("deviceId").toString();
    ...
    private void initData() {
        ...
        name = mapdevice.get("name") == null? null : mapdevice.get("name").toString();
        if(name != null)
...
//                        + uds + "_" + lrs + "_" + "p0";
                code_mode = mod + "_" + spe + "_" + temperature;
                break;
        }
        ...
        String normal_mode = code_mode;
        if (!code_is_contains(normal_mode)) return;
        normal_mode = normal_mode + "_" + uds;// up down speed
        if (code_is_contains(normal_mode)) {
            normal_mode = normal_mode + "_" + lrs + "_";
            if (!code_is_contains(normal_mode)) { // left right speed
                normal_mode = code_mode + "_" + uds + "__";
            }
        } else {
            normal_mode = code_mode + "__" + lrs + "_";
            if (!code_is_contains(normal_mode)) {
                normal_mode = code_mode + "___";
            }
        }
        normal_mode = normal_mode + "p0";
    ...
    private boolean code_is_contains(String code_mode) {
        Set<String> set = remoteControl_map_air.keySet();
        String key = null;
        for (String s : set) {
            if (s.contains(code_mode)) {
                key = s;
            }
        }
...
//                        + uds + "_" + lrs + "_" + "p0";
                                ...
                                if (remoteControl_map_air == null) { //w_s1_22___p0
                                    return;
                                }
...
//        }
//
        if (!code_is_contains(mode)) return;

        // 去掉 左右 上下扫风   在去控制


        code = remoteControl_map_air.get(mode).toString();
        code_up = code.substring(2);
        driverControl.sendCMD(code);
        onRefreshUI();
        voice_rise();
