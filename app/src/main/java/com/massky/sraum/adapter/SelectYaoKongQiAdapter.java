...
public class SelectYaoKongQiAdapter extends android.widget.BaseAdapter {
    ...
    public SelectYaoKongQiAdapter(AppCompatActivity context, List<Map> list, List<Integer> listint, List<Integer> listintwo, DialogUtil dialogUtil,
...
//                    delete_item();
                showCenterDeleteDialog(list.get(position).get("deviceId").toString(), list.get(position).get("number").toString(), list.get(position).get("device_name").toString());
            }
        });
...
//                refreshLayout.autoRefresh();
    ...
    private void sraum_deleteWifiApple(final String deviceId, final String number, final Dialog dialog) {
        ...
        mapdevice.put("token", TokenUtil.getToken(activity));
        mapdevice.put("number", number);
        String areaNumber = (String) SharedPreferencesUtil.getData(activity, "areaNumber", "");
