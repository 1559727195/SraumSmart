 ...
 * @decpter
 */
class CommonDialogService : CommonDialogListener {//
    private var belowtext_id: TextView? = null

    @Volatile
    private var _instance: CommonDialogService? = null

    fun getInstance(): CommonDialogService? {
        if (_instance == null) {
            synchronized(CommonDialogService::class.java) {
                if (_instance == null) {
                    _instance = CommonDialogService()
                }
            }
        }
        return _instance
    ...
    fun ondestroy() {
        ...
        ToastUtils.getInstances().releaseToast()
        if (_instance != null)
            _instance = null
    }
    ...
    private fun showDialog(str: String) {
        ...
        if (dialog == null && CommonData.mNowContext != null) {
            ...
            view = LayoutInflater.from(CommonData.mNowContext).inflate(R.layout.check_is_status, null, false)
            //belowtext_id
...
//            }
//        },100);
        if(CommonData.mNowContext == null) return
        init_jlogin(str)
    }
    ...
    private fun init_jlogin(str: String) {
        ...
        val TelephonyMgr = CommonData.mNowContext!!.getSystemService(TELEPHONY_SERVICE) as TelephonyManager
        //        String szImei = TelephonyMgr.getDeviceId();
        val szImei = SharedPreferencesUtil.getData(CommonData.mNowContext, "regId", "") as String
