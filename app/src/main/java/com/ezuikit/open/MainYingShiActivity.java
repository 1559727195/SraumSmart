...
import static com.ezuikit.open.DoublePlayYingShiActivity.TYPE;
import static com.ezuikit.open.PlayYingShiActivity.APPKEY;
import static com.ezuikit.open.PlayYingShiActivity.AccessToekn;
import static com.ezuikit.open.PlayYingShiActivity.Global_AreanDomain;
import static com.ezuikit.open.PlayYingShiActivity.PLAY_URL;

public class MainYingShiActivity extends Activity implements View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_main_ying_shi);
        isGlobal = getIntent().getBooleanExtra(PlayYingShiActivity.Global_AreanDomain, false);
        if (isGlobal) {
    ...
    public void onClick(View view) {
        if (view == mButtonCode) {
            ...
            if (urls != null && urls.length == 2) {
                ...
                if (isGlobal) {
                    //启动普通回放页面
                    DoublePlayYingShiActivity.startPlayActivity(this, mAppKey, mAccessToken, urls[0], urls[1], mGlobalAreaDomain);
                    return;
                }
                //启动普通回放页面
                DoublePlayYingShiActivity.startPlayActivity(this, mAppKey, mAccessToken, urls[0], urls[1]);
                return;
            }
            ...
            if (mode == EZUIPlayer.EZUIKitPlayMode.EZUIKIT_PLAYMODE_LIVE) {
                ...
                if (isGlobal) {
                    //启动播放页面
                    PlayYingShiActivity.startPlayActivityGlobal(this, mAppKey, mAccessToken, mUrl, mGlobalAreaDomain);
                    //应用内只能初始化一次，当首次选择了国内或者海外版本，并点击进入预览回放，此时不能再进行国内海外切换
                    return;
                }
                //启动播放页面
                PlayYingShiActivity.startPlayActivity(this, mAppKey, mAccessToken, mUrl);
            } else if (mode == EZUIPlayer.EZUIKitPlayMode.EZUIKIT_PLAYMODE_REC) {
                //回放
                if (mCheckBoxBack.isChecked()) {
                    if (isGlobal) {
                        //启动普通回放页面
                        PlayBackYingShiActivity.startPlayActivityGlobal(this, mAppKey, mAccessToken, mUrl, mGlobalAreaDomain);
                        //应用内只能初始化一次，当首次选择了国内或者海外版本，并点击进入预览回放，此时不能再进行国内海外切换
                        return;
                    }
                    //启动回放带时间轴页面
                    PlayBackYingShiActivity.startPlayBackActivity(this, mAppKey, mAccessToken, mUrl);
                } else {
                    if (isGlobal) {
                        //启动普通回放页面
                        PlayYingShiActivity.startPlayActivityGlobal(this, mAppKey, mAccessToken, mUrl, mGlobalAreaDomain);
    ...
    private void getDefaultParams() {
        ...
        mAppKey = sharedPreferences.getString(PlayYingShiActivity.APPKEY, "");
        mAccessToken = sharedPreferences.getString(PlayYingShiActivity.AccessToekn, "");
        mUrl = sharedPreferences.getString(PlayYingShiActivity.PLAY_URL, "");
        mGlobalAreaDomain = sharedPreferences.getString(PlayYingShiActivity.Global_AreanDomain, "");
...
//        mAccessTokenEditText.setText(mAccessToken);
    ...
    private void saveDefaultParams() {
        ...
        editor.putString(PlayYingShiActivity.APPKEY, mAppKey);
        editor.putString(PlayYingShiActivity.AccessToekn, mAccessToken);
        editor.putString(PlayYingShiActivity.PLAY_URL, mUrl);
        editor.putString(TYPE, mType);
        if (!isGlobal) {
            editor.putString(PlayYingShiActivity.Global_AreanDomain, "");
        } else {
            editor.putString(PlayYingShiActivity.Global_AreanDomain, mGlobalAreaDomain);
        }
        editor.commit();
    }
